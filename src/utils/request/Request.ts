import type { AxiosInstance, AxiosRequestHeaders, AxiosRequestConfig, AxiosResponse, AxiosInterceptorOptions, AxiosError } from 'axios';
...
class Request<R = any, D = any> {
   ...
   * @param type
   */
  clearInterceptors(type: 'request' | 'reponse' | 'all' = 'all') {
    if (type === 'request') {
  ...
  get<RES = R, DATA = D>(url: string, params: any = {}, config: RequestConfig<DATA> = {}) {
    return this.request<RES, DATA>({ method: 'get', url, params, ...config });
  }
  /**
   * post
   * @param url
  ...
  post<RES = R, DATA = D>(url: string, data: any = {}, config: RequestConfig<DATA> = {}) {
    return this.request<RES, DATA>({ method: 'post', url, data, ...config });
  }
  /**
   * put
   * @param url
  ...
  put<RES = R, DATA = D>(url: string, data: any = {}, config: RequestConfig<DATA> = {}) {
    return this.request<RES, DATA>({ method: 'put', url, data, ...config });
  }
  /**
   * patch
   * @param url
  ...
  patch<RES = R, DATA = D>(url: string, data: any = {}, config: RequestConfig<DATA> = {}) {
    return this.request<RES, DATA>({ method: 'patch', url, data, ...config });
  }
  /**
   * delete
   * @param url
  ...
  delete<RES = R, DATA = D>(url: string, data: any = {}, config: RequestConfig<DATA> = {}) {
    return this.request<RES, DATA>({ method: 'delete', url, data, ...config });
  }
}
